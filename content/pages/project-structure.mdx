...
1. **Clone and install**
   ```bash
   git clone https://github.com/your-org/muni-docs
   cd muni-docs
   pnpm install
   ```
...
1. **Create new content**
   - Add MDX files to `content/pages/`
   - Update navigation in `muni.config.ts`
   - Test locally with `pnpm dev`

2. **Modify components**
   - Edit components in `components/` or `mdx-components/`
   - Test changes in browser
   - Ensure TypeScript types are correct

3. **Update styles**
   - Modify CSS files in `styles/`
   - Test in all themes
...
1. **Components**: Group by functionality
   - `ui/` - Base UI components
   - `navigation-components/` - Navigation specific
   - `muni-components/` - Muni specific

2. **Content**: Organize by topic
   - Use descriptive filenames
   - Group related content in subdirectories
   - Use `index.mdx` for section homepages

3. **Styles**: Separate concerns
   - `globals.css` - Global styles
   - `color-themes.css` - Theme definitions
